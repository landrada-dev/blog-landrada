        ...
        <h4 class="media-heading">{{ $children->author->name }}
            <small>&#xB7;
                @if ($children->created_at->diffInDays(now()) > 0)
                    {{ $children->created_at->diffInDays(now()) }} {{ __(' Days Ago') }}
                @elseif ($children->created_at->diffInHours(now()) > 0)
                    {{ $children->created_at->diffInHours(now()) }} {{ __(' Hours Ago') }}
                @elseif ($children->created_at->diffInMinutes(now()) > 0)
                    {{ $children->created_at->diffInMinutes(now()) }} {{ __(' Minutes Ago') }}
                @else
                    {{ $children->created_at->diffInSeconds(now()) }} {{ __(' Seconds Ago') }}
                @endif
